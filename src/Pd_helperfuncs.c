...
void drawText(LCDFont* font, const char* text, size_t len, PDStringEncoding encoding, int x, int y)
{
	pd->graphics->setFont(font);
	pd->graphics->drawText(text, len, encoding, x, y);
}
...
bool FileExists(char* Path, bool datapath)
{
	bool result = false;
	FileOptions mode = kFileRead;
	if (datapath)
		mode = kFileReadData;
	SDFile* File = pd->file->open(Path, mode);
	if (File)
	{
		result = true;
		pd->file->close(File);
