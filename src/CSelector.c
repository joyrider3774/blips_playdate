...
CSelector *CSelector_Create(CWorldParts* List)
{
	...
	if (Result)
	{
		...
		Result->Part = CWorldPart_Create(0, 0, IDEmpty);
...
void CSelector_SetPosition(CSelector* Selector, int PlayFieldX, int PlayFieldY)
{
	CWorldPart_SetPosition(Selector->Part, PlayFieldX,PlayFieldY);
}
...
void CSelector_IncSelection(CSelector* Selector)
{
	...
	Selector->Part = CWorldPart_Create(X,Y, Selector->Selection);
	Selector->Part->ParentList = Selector->PList;
